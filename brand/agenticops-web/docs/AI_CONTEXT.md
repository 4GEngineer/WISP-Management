...
| Engine (product name on site) | **Chrysalis** | **GitHub + pilot alpha** | Repo: **https://github.com/4GEngineer/chrysalis** (org **4GEngineer**, not inside theorem6/WISP-Management). PHP-first verification migration; pilots scoped **one system at a time**. Technical narrative doc: **https://github.com/4GEngineer/chrysalis/blob/main/docs/AGENTICOP.md** (link from marketing site; if moved, sync `index.html`, `proof.html`, LinkedIn helper `CONFIG`, `LINKEDIN_POST_MAINTENANCE.md`). |
